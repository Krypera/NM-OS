...
| `app_overrides` | Per-application filesystem isolation overrides | `app_isolation_policy.py` per-app `flatpak override --system <app-id>` rules + Control Center per-app editor + settings D-Bus `SetOverrides` | Enforced (per-app filesystem profile layer) |
